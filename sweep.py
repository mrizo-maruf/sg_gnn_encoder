#!/usr/bin/env python3
"""
Hyperparameter sweep for Scene Graph GNN Encoder.

Searches over: model type, learning rate, hidden dimension,
...
from model import SceneGraphEncoder, SceneGraphEncoderLight
...
def compute_uniformity(z: torch.Tensor, t: float = 2.0) -> float:
    ...
    uniform = log E[ e^{-t * ||z_i - z_j||^2} ]

    Lower is better — embeddings spread uniformly on the hypersphere.
    A value near 0 means collapse (all embeddings identical).
    """
...
def evaluate_alignment_uniformity(
    ...
    For alignment: generates augmented positive pairs and measures distance.
    For uniformity: collects all scene embeddings and measures spread.

    Args:
        model: Trained encoder.
        dataset: Dataset to evaluate on.
        device: Compute device.
        n_aug_passes: How many augmented views to average alignment over.

    Returns:
        Dict with 'alignment', 'uniformity', 'combined' (align + uniformity).
    """
    model.eval()
    ...
    all_z = torch.cat(all_z, dim=0)  # [N_scenes, D]

    # ---- Uniformity ----
    uniformity = compute_uniformity(all_z)

    # ---- Alignment (average over multiple augmentation passes) ----
    alignments = []
    ...
    return {
        ...
        "combined": alignment + uniformity,  # lower is better for both
    }


# ──────────────────────────────────────────────────────────────────────
# Training helpers (streamlined for sweep)
# ──────────────────────────────────────────────────────────────────────
...
def train_config(
    ...
    full_dataset: SceneGraphDataset,
    device: torch.device,
    seed: int,
) -> dict:
    """
    ...
    ModelClass = SceneGraphEncoderLight if config["model"] == "2layer" else SceneGraphEncoder
    model = ModelClass(
    ...
    loss_fn = CombinedLoss(
        ...
        edge_hidden_dim=config["hidden_dim"],
        num_relations=6,
    ).to(device)
...
def plot_sweep_results(results: List[dict], out_dir: str) -> None:
    ...
    models_2l = [r for r in results if r["model"] == "2layer"]
    models_3l = [r for r in results if r["model"] == "3layer"]

    for group, marker, label in [(models_2l, "o", "2-layer"), (models_3l, "s", "3-layer")]:
        if not group:
            continue
        aligns = [r["alignment"] for r in group]
        uniforms = [r["uniformity"] for r in group]
        colors = [r["hidden_dim"] for r in group]
        sc = ax.scatter(uniforms, aligns, c=colors, marker=marker, s=100,
    ...
    ax.set_title("Alignment vs Uniformity — Hyperparameter Sweep", fontsize=13)
    ...
    for i, r in enumerate(sorted(results, key=lambda x: x["au_combined"])):
        ...
        bar_colors.append("tab:green" if r is best else ("tab:blue" if r["model"] == "3layer" else "tab:orange"))

    bars = ax.bar(range(len(au_scores)), au_scores, color=bar_colors, edgecolor="black", linewidth=0.5)
    ax.set_xticks(range(len(labels)))
    ax.set_xticklabels(labels, fontsize=7, ha="center")
    ax.set_ylabel("Alignment + Uniformity (lower = better)", fontsize=11)
    ax.set_title("Combined A+U Score per Configuration", fontsize=13)
    ...
    for model_type in ["2layer", "3layer"]:
        subset = [r for r in results if r["model"] == model_type]
        if not subset:
            continue
        ...
        for r in subset:
            ...
            li = lrs.index(r["lr"])
            # If multiple runs map here (different dropout/wd), keep best
            val = r["au_combined"]
        ...
        ax.set_title(f"A+U Score Heatmap — {model_type}", fontsize=13)
        ...
        plt.tight_layout()
        fig.savefig(str(plots_dir / f"heatmap_{model_type}.png"), dpi=150)
        plt.close(fig)

    # ---- 4. Regularization impact: dropout vs weight_decay ----
    dropouts = sorted(set(r["dropout"] for r in results))
    wds = sorted(set(r["weight_decay"] for r in results))
    ...
    if len(dropouts) > 1 or len(wds) > 1:
        ...
        # Dropout effect
        ax = axes[0]
        for model_type, marker in [("2layer", "o"), ("3layer", "s")]:
            subset = [r for r in results if r["model"] == model_type]
            if not subset:
                continue
            for do in dropouts:
                runs = [r for r in subset if r["dropout"] == do]
                if runs:
                    au_vals = [r["au_combined"] for r in runs]
                    ax.scatter([do] * len(au_vals), au_vals, marker=marker,
                              s=60, alpha=0.7, label=f"{model_type}" if do == dropouts[0] else "")
        # Mean line
        for do in dropouts:
            runs = [r for r in results if r["dropout"] == do]
            if runs:
        ...
        ax.set_title("Dropout Impact on Embedding Quality", fontsize=12)
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)
        ...
        for model_type, marker in [("2layer", "o"), ("3layer", "s")]:
            ...
            if not subset:
                continue
            for wd in wds:
                runs = [r for r in subset if r["weight_decay"] == wd]
                if runs:
                    au_vals = [r["au_combined"] for r in runs]
                    ax.scatter([wd] * len(au_vals), au_vals, marker=marker,
                              s=60, alpha=0.7, label=f"{model_type}" if wd == wds[0] else "")
        for wd in wds:
            runs = [r for r in results if r["weight_decay"] == wd]
            if runs:
                mean_au = np.mean([r["au_combined"] for r in runs])
                ax.plot(wd, mean_au, "D", color="red", markersize=10, zorder=5)
        ax.set_xlabel("Weight Decay", fontsize=11)
        ax.set_ylabel("A+U Score (lower = better)", fontsize=11)
        ax.set_title("Weight Decay Impact on Embedding Quality", fontsize=12)
        ax.set_xscale("log")
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)
    ...
    for i, r in enumerate(top_k):
        lbl = f"#{i+1} {r['model']} H={r['hidden_dim']} lr={r['lr']}"
        ax.plot(r["train_losses"], label=lbl, linewidth=1.2, alpha=0.8)
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Train Loss")
    ax.set_title("Training Curves — Top 5 Configs")
    ...
    ax.set_title("Validation Curves — Top 5 Configs")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
...
def build_search_grid(quick: bool = False) -> List[dict]:
    """
    Build the hyperparameter search grid.

    Full grid: 2 models × 3 lr × 3 hidden_dim × 2 dropout × 2 weight_decay = 72 configs
    Quick grid: 2 models × 2 lr × 2 hidden_dim × 2 dropout × 1 weight_decay = 16 configs
    """
    if quick:
        return list_configs(
            models=["2layer", "3layer"],
            lrs=[5e-4, 1e-3],
            hidden_dims=[40, 64],
            dropouts=[0.2, 0.3],
            weight_decays=[1e-4],
        ...
        return list_configs(
            models=["2layer", "3layer"],
            lrs=[3e-4, 5e-4, 1e-3],
            hidden_dims=[32, 40, 64],
            dropouts=[0.15, 0.25, 0.35],
            weight_decays=[1e-4, 5e-4],
        )


def list_configs(models, lrs, hidden_dims, dropouts, weight_decays) -> List[dict]:
    configs = []
    for model, lr, hd, do, wd in itertools.product(models, lrs, hidden_dims, dropouts, weight_decays):
        configs.append({
            "model": model,
            "lr": lr,
            "hidden_dim": hd,
...
def main():
    parser = argparse.ArgumentParser(description="Hyperparameter sweep for SG-GNN Encoder")
    parser.add_argument("--data_dir", type=str, required=True, help="Path to JSON scene graph directory")
    parser.add_argument("--out_dir", type=str, default="sweeps", help="Output directory")
    ...
    print(f"[sweep] Data: {len(train_dataset)} train, {len(val_dataset)} val, {len(full_dataset)} total")
    print(f"[sweep] Output: {sweep_dir}")

    # ---- Build grid ----
    search_grid = build_search_grid(quick=args.quick)
    n_configs = len(search_grid)
    print(f"[sweep] Grid size: {n_configs} configurations")
    print(f"[sweep] Epochs per config: {args.epochs}")
    print(f"[sweep] Estimated total epochs: {n_configs * args.epochs}")
    ...
    csv_writer.writerow([
        "rank", "model", "hidden_dim", "lr", "dropout", "weight_decay",
    ...
    for i, config in enumerate(search_grid):
        ...
        print(f"\n{'─'*60}")
        print(f"  Config {i+1}/{n_configs}: {config['model']} | "
              f"H={config['hidden_dim']} | lr={config['lr']} | "
              f"do={config['dropout']} | wd={config['weight_decay']}")
        print(f"{'─'*60}")
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for rank, r in enumerate(results, 1):
            writer.writerow([
                rank, r["model"], r["hidden_dim"], r["lr"], r["dropout"],
                r["weight_decay"], r["n_params"], f"{r['best_val_loss']:.6f}",
    ...
    print(f"{'Rank':<5} {'Model':<8} {'H':<5} {'LR':<10} {'DO':<6} {'WD':<10} "
          f"{'Align':>8} {'Uniform':>9} {'A+U':>8} {'ValLoss':>9}")
    print("─" * 90)
    for rank, r in enumerate(results, 1):
        marker = " ★" if rank == 1 else ""
        print(f"{rank:<5} {r['model']:<8} {r['hidden_dim']:<5} {r['lr']:<10.0e} "
              f"{r['dropout']:<6.2f} {r['weight_decay']:<10.0e} "
    ...
    print(f"       ├── heatmap_2layer.png")
    print(f"       ├── heatmap_3layer.png")
    ...
    print(f"  python train.py --data_dir {args.data_dir} --out_dir runs "
          f"--model {best['model']} --hidden_dim {best['hidden_dim']} "
          f"--lr {best['lr']} --dropout {best['dropout']} "
          f"--weight_decay {best['weight_decay']} --epochs 200 --seed {args.seed}")
